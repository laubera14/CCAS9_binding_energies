...
my $s_5prime = 22; #specify how many bases should be cut away from 5'-End (4nt-gRNA-PAM)
...
for (my $round = 0; $round<10; $round+= 2){
$count = 0;
$s_5prime -= $round;
...
my $nt_count = 19 - $round;
my $filetowrite_energy_difference = "binding_ediffsum_PAM_20_$nt_count.txt"; #specify where energy differences should be written
my $p = $nt_count + 2;
my $previous_energy_file;
if ($round > 0){
$previous_energy_file = "binding_ediffsum_PAM_20_$p.txt";
}
...
while (defined(my $hybrid_row = <$read_hybrid_file>) and defined(my $DNA_row = <$read_DNA_file>)) {
 ...
 if ($DNA_row=~m/^(.+)\s+(\d+\,\d+)\s+(\d+\,\d+)\s+(-?\d+\.\d+)\s+(\d+\.\d+|0)\s+(\d+\.\d+)/){
   $DNA_energy = $4;
   $energy_difference = ($hybrid_energy-$DNA_energy)*$factors[$round];
...
open (my $read_previous_energy_file, '<:encoding(UTF-8)', $previous_energy_file)
  or die "Could not open file '$previous_energy_file' $!";

open($filestream, '>', $filetowrite_energy_difference) or die "Could not write file '$filetowrite_energy_difference' $!";
print $filestream "delta_e\tsgScore\trank\tsequence\n";

while (defined(my $energy_row = <$read_previous_energy_file>)) {

if ($energy_row=~m/^(-?\d+\.\d+)/){
  $previous_energy = $1;

if (defined(my $hybrid_row = <$read_hybrid_file>) and defined(my $DNA_row = <$read_DNA_file>)){
  if ($hybrid_row=~m/^([ATGCU]+-[ATGCU]+)/){
    $sequence = $1;
  ...
  if ($DNA_row=~m/^(.+)\s+(\d+\,\d+)\s+(\d+\,\d+)\s+(-?\d+\.\d+)\s+(\d+\.\d+|0)\s+(\d+\.\d+)/){
    $DNA_energy = $4;
    $energy_difference = ($hybrid_energy-$DNA_energy)*$factors[$round];
    $new_energy = $previous_energy + ($energy_difference - $previous_energy);
      printf $filestream "%.4f", "$new_energy";
      printf $filestream "\t";
      printf $filestream "%.3f", "$sgRNAScore";
      printf $filestream "\t";
      printf $filestream "%.3f", "$geneRank";
...
close $read_hybrid_file;
close $read_DNA_file;
close $read_previous_energy_file;
close $filestream;
print "calculated free energy difference of $count sequences and wrote to file $filetowrite_energy_difference\n";
}
